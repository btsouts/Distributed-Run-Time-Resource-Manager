	...
	if (node_id == 3) { 
		...
		while (state != IDAG_ENDING)
			...
			/*} else if (state == FAR_MAN_CHK_OFFERS){
				...
				/*while (ans != 1){
					printf("give next command: 1 to term 2 to start new_app 3 to req DDS: \n");
					scanf("%d",&ans);
					if (ans == 1) {
						printf("killing\n");
						for (i=1; i<num_idags; i++) {
							...
							if (core_inter_head[one_core] == NULL){
								...
								core_inter_tail[one_core]->next = (inter_list *) malloc(sizeof(inter_list));
								core_inter_tail[one_core] = core_inter_tail[one_core]->next;
								printf("I am %d and i am still doing smth with idag %d interaction = %d\n",node_id,one_core,core_inter_head[one_core]->type);
							}
						...
						for (; my_cores != NULL; my_cores = my_cores->next){
							...
							if (core_inter_head[one_core] == NULL){
								...
								core_inter_tail[one_core]->next = (inter_list *) malloc(sizeof(inter_list));
								core_inter_tail[one_core] = core_inter_tail[one_core]->next;
								printf("I am %d and i am still doing smth with my node %d interaction = %d\n",node_id,one_core,core_inter_head[one_core]->type);
							}
						...
						state = IDAG_ENDING;
							//else if (node_id == 0 && stats_replied == my_cores_count-1) state = TERMINATED; 
					} else if (ans == 2) {
						printf("Give me idag to request DDS: ");
						scanf("%d",&one_idag);

						printf("Req dds from %d\n",one_idag);
						if (one_idag != 0){
							...
							if (core_inter_head[one_idag]->next == NULL) kill(pid_num[one_idag], SIG_REQ_DDS);
